    ...
    def calculer_supplement_nuit_ou_ferie(self):
     ...
     if self.heure_debut and self.heure_fin:
        ...
        liste_jours_fériés = [  # Liste des dates de jours fériés (à ajuster)
            datetime.date(year=2024, month=4, day=26),  # Exemple pour aujourd'hui (si aujourd'hui est un jour férié)
        ]
        ...
        nuit = (heure_debut_demande >= heure_debut_nuit or heure_debut_demande < heure_fin_nuit) \
               and (heure_fin_demande > heure_debut_nuit or heure_fin_demande <= heure_fin_nuit)

        if (nuit_passée and nuit) or (not nuit_passée and (heure_debut_demande >= heure_debut_nuit or heure_fin_demande <= heure_fin_nuit)):
            supplément = 0.2  # Définir le supplément pour la nuit
        
        date_demande = self.date
        if date_demande in liste_jours_fériés:
            supplément = 0.3  # Définir le supplément pour jour férié
